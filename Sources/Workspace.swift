...
extension Workspace {
    func sessionSnapshot(
        ...
        // When this is a remote SSH workspace that maps to a saved server in the
        // Server Library, record its id so restore can re-stamp the library's
        // "last connected" recency. The SSH reconnect itself is driven by the
        // `remote` snapshot below; tmux reattach is per-panel via
        // `remotePTYSessionID`, so no separate session name is captured here.
        let savedServerId: UUID? = {
            ...
            return ServerLibraryStore.shared.server(matchingDestination: configuration.destination)?.id
        }()
    ...
    func restoreSessionSnapshot(_ snapshot: SessionWorkspaceSnapshot) -> [UUID: UUID] {
        ...
        if let restoredRemoteConfiguration {
            ...
            // If this workspace was opened from a Server Library entry, keep the
            // library's "last connected" recency accurate across restore. The SSH
            // connection is re-established by `configureRemoteConnection` above and
            // per-panel tmux reattach is driven by `remotePTYSessionID`, so the
            // saved-server fields only need to refresh recency here.
            if shouldAutoConnect, let savedServerId = snapshot.savedServerId {
                ServerLibraryStore.shared.markConnected(id: savedServerId)
            }
        } else {
