...
struct SessionWorkspaceSnapshot: Codable, Sendable {
    ...
    /// The `SavedServer.id` (from the server library) that this workspace was
    /// connected to when the snapshot was captured. When present alongside
    /// `tmuxSessionName`, restore can auto-reconnect the saved server and
    /// re-attach the named tmux session ("save server + tmux session as a
    /// workspace"). Optional with a `nil` default so snapshots written before
    /// this field — and snapshots of purely local workspaces — decode unchanged.
    var savedServerId: UUID? = nil
    /// The remote tmux session name to re-attach on restore. Optional with a
    /// `nil` default for the same backward-compatibility reasons as
