...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation, NSMenuDelegate, CmuxConfigStoreReloadEnvironment {
    ...
    func application(_ application: NSApplication, open urls: [URL]) {
        ...
        #endif
        // remux:// collaboration invites: drop into the same shared tmux session.
        if handleRemuxInviteURLs(from: urls) {
            return
        }
    ...
    /// Handles `remux://join?host=&session=&ts=` collaboration invites by
    /// confirming with the user, then attaching the shared tmux session. Returns
    ...
    @MainActor
    private func confirmRemuxInvite(_ invite: RemuxCollabSession.Invite) -> Bool {
        let alert = NSAlert()
        alert.messageText = String(localized: "collab.invite.confirmTitle", defaultValue: "Join shared session?")
        alert.informativeText = String.localizedStringWithFormat(
            String(localized: "collab.invite.confirmBody", defaultValue: "Connect to %1$@ and attach the shared session \u{201C}%2$@\u{201D}?"),
            invite.host, invite.session
        )
        alert.addButton(withTitle: String(localized: "collab.invite.join", defaultValue: "Join"))
        alert.addButton(withTitle: String(localized: "collab.invite.cancel", defaultValue: "Cancel"))
        return alert.runModal() == .alertFirstButtonReturn
    }
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        DispatchQueue.main.async {
            ...
            alert.messageText = String(localized: "dialog.quitCmux.title", defaultValue: "Quit remux?")
        ...
        ghosttyCrashBreadcrumbTask = Task { [weak self, weak notificationStore] in
            ...
            notificationStore.addNotification(
                ...
                title: String(
                    ...
                    defaultValue: "remux crashed during your last session"
                ),
                subtitle: String(
